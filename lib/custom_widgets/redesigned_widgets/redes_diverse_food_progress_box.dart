  ...
  if (state is DiverseFoodLoadedState) {
    ...
    return Container(
      ...
      child: _redesDiverseFoodOnlyProgressBox(
          ...
          state: state,
          context: context,
          countPurchases: state.diverseFoodStatsModel.data.isEmpty
              ? 0
              : state.diverseFoodStatsModel.data.first.purchasedCount,
          requiredPurchases: futurePuchases,
          ratedCount: state.diverseFoodStatsModel.data.isEmpty
              ? 0
              : state.diverseFoodStatsModel.data.first.ratedCount,
    ...
    children: [
      ...
      if (state is DiverseFoodLoadedState)
        Padding(
          ...
          child: Center(
            child: Text(
              (requiredRatedCount - ratedCount) > 0 ||
                      (requiredPurchases - countPurchases) > 0
                  ? "Осталось купить ${requiredPurchases - countPurchases < 0 ? "0" : (requiredPurchases - countPurchases).toString()} товаров" +
                      (requiredRatedCount - ratedCount > 0
                          ? " ${"andEvaluateText".tr()} ${(requiredRatedCount - ratedCount).toString()} из них"
                          : "")
      ...
      children: [
        Container(
            ...
            decoration: BoxDecoration(
              color: percent < minDiscountPercent ||
                      (percent == minDiscountPercent &&
                          doneProgress >= requiredProgress &&
                          isLast)
                  ? newRedDark
                  : newIconBg,
        ...
        Container(
          ...
          decoration: BoxDecoration(
            borderRadius:
                BorderRadius.circular(heightRatio(size: 25, context: context)),
            color: newIconBg2,
          ),
