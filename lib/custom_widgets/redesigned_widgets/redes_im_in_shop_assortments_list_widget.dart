...
class RedesImInShopAssortmentsListWidget extends StatefulWidget {
  @override
  State<RedesImInShopAssortmentsListWidget> createState() =>
      _RedesImInShopAssortmentsListWidgetState();
}

class _RedesImInShopAssortmentsListWidgetState
    extends State<RedesImInShopAssortmentsListWidget> {
  int i = 1;
    ...
    return BlocBuilder<ImInShopBloc, ImInShopState>(builder: (context, state) {
      if (state is ImInShopLoadedState) {
        ...
        for (var i = 0; i < state.imInShopModel.data.products.length; i++) {
          _imInShopAssortmentList
              .add(state.imInShopModel.data.products[i].uuid);
        }
        return Container(
          clipBehavior: Clip.none,
          color: Colors.transparent,
          height: !state.imInShopModel.data.products.isEmpty
              ? screenWidth(context) / 2.9
              : 0,
          child: ListView.builder(
            ...
            itemBuilder: (context, index) => Container(
              ...
              child: RedesAssortmentsCard2Widget(
                ...
                assortmentsListModel: mapImInShopProductToAssortmentProduct(
                    product: state.imInShopModel.data.products[index]),
              ),
            ),
          ),
        );
...
mapImInShopProductToAssortmentProduct({required Product product}) {
