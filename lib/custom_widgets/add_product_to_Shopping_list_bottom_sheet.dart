...
class AddProductToShoppingListBottomSheetwidget extends StatefulWidget {
  ...
  @override
  _AddProductToShoppingListBottomSheetwidgetState createState() =>
      _AddProductToShoppingListBottomSheetwidgetState();
}

class _AddProductToShoppingListBottomSheetwidgetState
    extends State<AddProductToShoppingListBottomSheetwidget> {
  ...
  Widget build(BuildContext context) {
    ...
    Future<bool> deleteShoppingListToAdded(String uuid) async {
      return await DeleteProductToShopingListProvider()
          .getDeleteProducttoShoppingListRespone(
              shoppingListsUuid: uuid, assortmentUuid: widget.assortmentUuid);
    }
                          ...
                          ? Flexible(
                              child: Container(
                                child: ListView.builder(
                                  itemCount:
                                      state.shoppingListsModel.data.length,
                                  itemBuilder:
                                    ...
                                    return Container(
                                      child: Row(
                                        ...
                                        children: [
                                          Text(
                                            state.shoppingListsModel.data[index]
                                                .name,
                                            style: appTextStyle(
                                          ...
                                          Checkbox(
                                              onChanged: (value) async {
                                                ...
                                                if (value == true) {
                                                  if (await addShoppingListToAdded(
                                                      state.shoppingListsModel
                                                          .data[index].uuid)) {
                                                    widget.productDetailsBloc.add(
                                                        ProductDetailsLoadEvent(
                                                      uuid:
                                                          widget.assortmentUuid,
                                                    ));
                                                  }
                                                } else {
                                                  if (await deleteShoppingListToAdded(
                                                      state.shoppingListsModel
                                                          .data[index].uuid)) {
                                                    widget.productDetailsBloc.add(
                                                        ProductDetailsLoadEvent(
                    ...
                    child: Row(
                      ...
                      children: [
                        Text(
                          ...
                          style: appLabelTextStyle(
                              fontSize: heightRatio(
                                  size: heightRatio(size: 18, context: context),
                                  context: context),
                              fontWeight: FontWeight.w400),
                        ),
                        GestureDetector(
                          onTap: () =>
                              _openBottomSheetNewShoppingListCrater(context),
                          child: Container(
