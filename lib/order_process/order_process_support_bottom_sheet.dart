...
class OrderProcessSupportBottomSheet extends StatelessWidget {
  ...
  const OrderProcessSupportBottomSheet({
          ...
          child: Column(
            ...
            children: [
              ...
              Text('Номер вашего заказа: $orderNumber',
                  style: appHeadersTextStyle(fontSize: 14, color: newBlack)),
              SizedBox(height: heightRatio(size: 20, context: context)),
              Padding(
                padding: EdgeInsets.symmetric(
                    horizontal: widthRatio(size: 30, context: context)),
                child: Text(
                    'При обращении в поддержку не забудьте сообщить номер вашего заказа',
                    style: appLabelTextStyle(
                        fontSize: 14, color: newBlack, height: 1.2),
              ...
              InkWell(
                onTap: () async {
                  String? whatsapp = await prefs
                      .getString(SharedKeys.socialsListModelUrlWhatsapp);
