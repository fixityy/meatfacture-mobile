...
class FavoriteProductLoadedState extends FavoriteProductState {
  ...
  FavoriteProductLoadedState({
    required this.favoriteProductVariantUuidModel,
    this.favoriteProductTitleModel,
    this.purchasesSumModel,
    required this.favoriteProductModel,
  });
}
...
class FavoriteProductBloc
  ...
  @override
  Stream<FavoriteProductState> mapEventToState(
      FavoriteProductEvent event) async* {
    if (event is FavoriteProductLoadEvent) {
      ...
      try {
        ...
        final FavoriteProductModel _favoriteProductModel =
            await FavoriteProductProvider().getFavoriteProductResponse();
        FavoriteProductTitleModel _favoriteProductTitleModel =
            await PromoDescriptionsProvider().favoritePrductTitleRequest();
        String _purchasesSumModelfor1Day =
            await ProfileProvider().getPurchasesSumResponse(days: 1);

        yield FavoriteProductLoadedState(
            purchasesSumModel: _purchasesSumModelfor1Day,
            favoriteProductVariantUuidModel: _favoriteProductVariantUuidModel,
            favoriteProductModel: _favoriteProductModel,
            favoriteProductTitleModel: _favoriteProductTitleModel);
      } catch (_) {
