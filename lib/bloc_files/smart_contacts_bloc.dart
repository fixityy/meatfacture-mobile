...
class SmartContactsLoadedState extends SmartContactsState {
  ...
  SmartContactsLoadedState({
    required this.socialsListModel,
    required this.smartContactsModel,
...
class SmartContactsBloc extends Bloc<SmartContactsEvent, SmartContactsState> {
  ...
  Stream<SmartContactsState> mapEventToState(SmartContactsEvent event) async* {
    ...
    try {
      SmartContactsModel _smartContactsModel =
          await SmartContactsProvider().getContactsResponse();
      prefs = await SharedPreferences.getInstance();
      await prefs.setString(SharedKeys.callCenterNumber,
          _smartContactsModel.data.callCenterNumber);
      log('попап отображать: ');
      await prefs.setBool(SharedKeys.isSocialNetworkFacebook,
          _smartContactsModel.data.socialNetworkFacebook == '1' ? true : false);
      log('попап отображать: ${prefs.getBool(SharedKeys.isSocialNetworkFacebook)}');

      SocialsListModel _socialsListModel =
          await SmartContactsProvider().getSocialsListResponse();
      await prefs.setString(SharedKeys.socialsListModelUrlWhatsapp,
          _socialsListModel.data[0].url);

      PackageInfo packageInfo = await PackageInfo.fromPlatform();
      yield SmartContactsLoadedState(
